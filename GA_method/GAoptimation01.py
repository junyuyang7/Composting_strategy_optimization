...
class NSGA_II:
    ...
    def get_min_max(self):
        ...
        for i, fea in enumerate(fix_fea):
            self.raw_dict[fea] = [self.fix_nums[0]]
        # self.raw_dict["Initial TN (%)"] = ref_mean_tbl.loc[ref_mean_tbl['Material_Main'] == self.fix_nums[0] , "Initial TN (%)"].values[0]
        # self.raw_dict["Compost volume (m3)"] = ref_mean_tbl.loc[ref_mean_tbl['Material_Main'] == self.fix_nums[0] , "Compost volume (m3)"].values[0]
        # self.raw_dict["Additive_3"] = [4]
        # self.raw_dict["Additive_4"] = [1]
        
        print(f'{sys._getframe().f_code.co_name} finish')
        return min_max_values
    
    @staticmethod
    def re_map(inidi, input_ranges):
        ref_mean_tbl = pd.read_csv("forecast_result/data/fore_data/ref_mean_tbl.csv")
        ...
        # 13.turning times <= priod
        condition = inidi["Turning times"] > inidi["Period (d)"]
        if condition:
            inidi["Turning times"] = random.randint(input_ranges["Turning times"][0], inidi["Period (d)"])
    ...
    def create_individual(self, input_ranges, toolbox):
        ...
        for attr_name, attr_info in input_ranges.items():
            if attr_name in int_feas:
                ...
                individual[attr_name] = toolbox.attr_int(attr_info)
        individual = self.re_map(individual, input_ranges)
    ...
    def optimization(self, targets, population_size=200, n_generations=100, cxpb=0.5, mutpb=0.01, num_runs=200, new_dict=None):
        # 创建输入范围字典和输出范围字典
        cols = []
        ...
        for i in range(num_runs):
            ...
            # 暂时保存
            self.save_res(f"{self.save_path}individuals_{targets}_{self.fix_nums}_tmp.csv", targets, input_ranges, best_individuals)
            print(f"Run {i+1} Saved")

        # 保存最优个体到CSV文件
        file_name = f"{self.save_path}individuals_{targets}_{self.fix_nums}.csv"
        self.save_res(file_name, targets, input_ranges, best_individuals)
        print(f'{sys._getframe().f_code.co_name} finish')
    ...
    def save_res(self, file_name, targets, input_ranges, best_individuals):
